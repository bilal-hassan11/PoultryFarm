...
<tr class="text-dark">
                                    <td>{{ date('m-D-Y', strtotime($out->date)) }}</td>
                                    <td>{{ $out->vehicle_no }}</td>
                                    <?php 
                                          if($loop->iteration == 1){
                                            $prev_gp_no = $out->gp_no;
                                          }else{
                                            
                                            if(isset($prev_gp_no)){
                                              if($prev_gp_no == $out->gp_no){
                                                $prev_gp_no = "";
                                              }else{
                                                $prev_gp_no = $out->gp_no;
                                              }
                                    ...
                                    ?>
                                    <td>{{ $prev_gp_no }}</td>
                                    <?php $new_no = $out->gp_no ?>
                                    <td>{{ @$out->account->name }}</td>
                                    <td>{{ $out->sub_dealer_name }}</td>
                                    <td>{{ @$out->item->name }}</td>
                                    <td>{{ @$out->no_of_begs    }}</td>
                                    <td>{{ $out->fare }}</td>
                                    <td>{{ number_format(@$out->item->price * @$out->no_of_begs) }}</td>
                                    
                                    <td width="120">
                                        <a href="{{route('admin.sales.edit', @$out->id)}}" >
                                        <span class="waves-effect waves-light btn btn-rounded btn-primary-light"><i class="fas fa-edit"></i></span>

                                        </a>
                                        <button type="button" onclick="ajaxRequest(this)" data-url="{{ route('admin.sales.delete', ['gp_no'=>@$out->hashid]) }}"  class="waves-effect waves-light btn btn-rounded btn-warning-light">
