...
class FeedInvoiceController extends Controller
{
    ...
    public function createPurchase(Request $req)
    {
        ...
        $purchase_Feed = FeedInvoice::with('account', 'item','shade')
            ->where('type', 'Purchase')
        ...
        $pending_Feed = FeedInvoice::with('account', 'item','shade')
            ->where('type', 'Purchase')
            ->where('net_amount', 0)
            ->latest()
            ->get();

        return view('admin.feed.purchase_feed', compact(['title','shade', 'pending_Feed', 'invoice_no', 'accounts', 'products', 'purchase_Feed']));
    }
    ...
    public function editPurchase($invoice_no)
    {
        ...
        return view('admin.feed.edit_purchase_feed', compact(['title','shade', 'accounts', 'products', 'FeedInvoice']));
    }
    public function editSale($invoice_no)
    {
        ...
        return view('admin.feed.edit_sale_feed', compact(['title','shade', 'pending_Feed', 'accounts', 'products', 'FeedInvoice']));
    }
    ...
    public function createSale(Request $req)
    {
        ...
        $shade = Shade::latest()->get();
        $sale_Feed = $FeedInvoice::with('account', 'item','shade')
            ->where('type', 'Sale')
            ->when(isset($req->account_id), function ($query) use ($req) {
        ...
        return view('admin.feed.sale_feed', compact(['title','shade', 'pending_Feed', 'sale_Feed', 'invoice_no', 'accounts', 'products']));
    }
        ...
        try {
            ...
            foreach ($items as $index => $itemId) {
                ...
                $FeedInvoice = FeedInvoice::create([
                    'date' => $date,
                    'account_id' => $validatedData['account'] ?? NULL,
